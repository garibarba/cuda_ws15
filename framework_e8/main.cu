...
__device__ float* eigval(float m11, float m12, float m21, float m22) {
  // implementing solution for: lambda**2 + lambda*(-m22-m11) + (a11*a22)
  // a = 1
  float b = - m11 - m22;
  float c = m11*m22 - m12*m21;
  float *eigenval;
  eigenval = (float[2]) { (-b - sqrtf(b*b - 4*c))/2, (-b + sqrtf(b*b - 4*c))/2 };

  return eigenval;
}

// this code is only expected to work with RGB images
// imgOut should have been initialized to 0
__global__ void gpu_edges_corners(float *imgOut, float *imgIn, float *M11, float *M12, float *M22, float alpha, float beta, int w, int h, int nc) {
  // calculate center pixel corresponding to thread
  int x = threadIdx.x + blockDim.x * blockIdx.x;
  int y = threadIdx.y + blockDim.y * blockIdx.y;
  if (x < w && y < h) {
    int idx = x + y*w;
    float *eigenval = eigval(M11[idx], M12[idx], M12[idx], M22[idx]);
    if ( eigenval[0] >= alpha ) {
      imgOut[idx] = 1; // red
    } else if ( eigenval[0] <= beta && eigenval[1] >= alpha ) {
      // R+G = Y
      imgOut[idx] = 1; // red
      imgOut[idx + w*h] = 1; // green
    } else {
      for (int c = 0; c < nc; c++)
        imgOut[idx + c*w*h] = 0.5*imgIn[idx + c*w*h];
    }
  }
}
...
int main(int argc, char **argv)
{
    ...
    if (alpha <= beta)
      cout << "!!! alpha should be > beta" << endl;
